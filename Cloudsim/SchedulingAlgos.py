...
def irr(workerList, tasks, scheduler):
    ...
    for each in scheduler.avgRtPerWorker.values():
        weights.append (each[0] - scheduler.avgRT[0]+randint(1,10))
    ...
    if (len(weights) == len(workerList)):
       ...
       if len(weights)==0:
           for g in range(7):
               weights=numpy.random.randint(15, size=7)

       #print("wei",weights)
       minWeight = numpy.argmin (weights)
       ...
       normalisedList = list (map(lambda x : x + 2*minWeight, weights) )# add 1 so that all weights are non-zero
       minWeight = min(normalisedList)
       normalisedList =  list(map (lambda x : x/minWeight, normalisedList))
    ...
    if (usingWeights == 1):
        ...
        i = 0
        for each in newList:
            normalisedList=list(normalisedList)
