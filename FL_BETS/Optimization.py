...
def Algm(data,enc_data,key):
    ...
    def initialize_SPC(N): # self-position consideration (SPC)
        spc=[]
        for i in range(N):
            spc.append(random.randint(0,key))
        return spc
    ...
    N , M, Tmax, g =  5,5,10, 0 # rows(cats), column size, maximum iteration
    ...
    while (g < Tmax):
        ...
        overall_best.append(solution[0])

        SMP = solution.copy() # seeking memory pool
        velocity = velocity_update(velocity, solution[0], solution)
